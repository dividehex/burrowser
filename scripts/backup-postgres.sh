#!/usr/bin/env bash
# Dumps the burrowser database (identities, profiles, invitations,
...
BACKUP_DIR="${BURROWSER_BACKUP_DIR:-./backups}"
...
filename="burrowser-$timestamp.dump"
out="$BACKUP_DIR/$filename"

docker exec -e PGPASSWORD="$BURROWSER_DB_PASSWORD" "$POSTGRES_CONTAINER" \
  pg_dump -U "$BURROWSER_DB_USER" -d "$BURROWSER_DB_NAME" -F custom -f "/tmp/$filename"
