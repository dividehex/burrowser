...
test('Kubernetes worker Secret provider decodes and validates expected keys', async () => {
  ...
  const provider = new KubernetesWorkerSecretProvider({ get: async (_kind, name) => { assert.equal(name, workerSecretName(profile)); return { data: { WORKER_CONTROLLER_CREDENTIAL: Buffer.from('controller').toString('base64'), AGENT_BROWSER_AUTHENTICATOR_KEY: Buffer.from(key).toString('base64'), AGENT_BROWSER_VNC_PASSWORD: Buffer.from('vncpass').toString('base64') } }; }, apply: async () => {}, delete: async () => {}, podStatus: async () => undefined });
...
test('Kubernetes worker Secret provider fails closed on missing or malformed Secrets', async () => {
  ...
  kube.get = async () => ({ data: { WORKER_CONTROLLER_CREDENTIAL: Buffer.from('controller').toString('base64'), AGENT_BROWSER_AUTHENTICATOR_KEY: Buffer.from('short').toString('base64'), AGENT_BROWSER_VNC_PASSWORD: Buffer.from('vncpass').toString('base64') } });
  await assert.rejects(() => new KubernetesWorkerSecretProvider(kube).get(profile), /Secret malformed/);
  kube.get = async () => ({ data: { WORKER_CONTROLLER_CREDENTIAL: Buffer.from('controller').toString('base64'), AGENT_BROWSER_AUTHENTICATOR_KEY: Buffer.from('short').toString('base64') } });
...
test('Kubernetes worker Secret provider creates a fresh Secret exactly once', async () => {
  ...
  assert.equal((created.resource as any).stringData.AGENT_BROWSER_VNC_PASSWORD, material.vncPassword);
});
