...
test('credentials persist encrypted and authenticate on reload', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'agent-browser-')); const path = join(dir, 'authenticator', 'credentials.enc'); const key = Buffer.alloc(32, 7); const records: any = [{ rpId: 'example.com', credentialId: 'id', userHandle: 'user', publicKey: 'pub', privateKey: 'secret' }];
